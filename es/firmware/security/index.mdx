...
| `0x0068+` | 124 B | Metadatos TOTP (algoritmo + longitud del secreto, 2 B × 62 slots) |
| `0x0100+` | — | Páginas de credenciales (4 × 32 B × 62 slots = 7936 B máx.) |
...
2. Para cada uno de los 62 slots × 4 páginas: cifra un blanco de 32 bytes `0xFF` bajo AES-128 CBC y lo escribe en EEPROM.
...
- **Export (`backupAllCredentials`):** descifra los 62 slots dentro del dispositivo y los envía como líneas plaintext separadas por comas por SerialUSB. El host recibe las credenciales en claro — **asegúrate de que la conexión USB es de confianza**.
