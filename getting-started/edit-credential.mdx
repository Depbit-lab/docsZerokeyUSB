...
For password fields, instead of typing letter by letter, use **Rand**. The value comes straight from the ATECC608A's hardware TRNG (never a software PRNG) and **never exceeds 16 characters**.
...
|---------------|--------|---------|
| `Symbols` (default) | 16 chars from the full printable set (letters, digits, symbols) | `t7#Kp!2r$Qm9^Za5` |
| `Numeric` | 16 digits | `4820193756042817` |
| `a-z 0-9` | 16 lowercase letters + digits | `k3m9xq1z7r4p2n8w` |
| `Aa-z 0-9` | 16 mixed-case letters + digits | `Kp3Mx9Qz1Rt4Nb8y` |
| `Words` | Capitalised BIP39 words joined by a random separator (`-_.!@#*+`) | `Ocean-Cargo-Mint` |
| `Words+Num` | Same, plus a 2-digit group after the first word | `River_28_Mango` |

The word formats reuse the built-in BIP39 wordlist and greedily pack as many words as fit under 16 characters. `Rand` on the **site** or **user** fields always uses the full `Symbols` set regardless of this setting.
