...
For password fields, instead of typing letter by letter, use **Rand**. The value comes straight from the ATECC608A's hardware TRNG (never a software PRNG) and **never exceeds 32 characters**.
...
The word formats reuse the built-in BIP39 wordlist and greedily pack as many words as fit under 32 characters. `Rand` on the **site** or **user** fields always uses the full `Symbols` set regardless of this setting.
