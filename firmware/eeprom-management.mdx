...
block-beta
    columns 1
    block:config["Configuration Zone (0x0000–0x00DF)"]
        ...
        G["0x0028–0x0037: AES master key (16B)"]
        H["0x0038–0x0057: PIN hash (32B)"]
        I["0x003E: Keyboard layout (1B)"]
        J["0x0040–0x0047: Last TOTP epoch (8B)"]
        K["0x0060–0x00DB: TOTP meta (124B)"]
    end
    block:creds["Credential Zone (0x00E0–0x1FFF)"]
        L["62 credential slots × 128 bytes each"]
    end
...
| `0x0028–0x0037` | 16 B | AES-128 master key (TRNG-generated) | `EEPROM_AES_MASTER` |
| `0x0038–0x0057` | 32 B | PIN hash: SHA-256(PIN ∥ serial) | `EEPROM_PIN_HASH` |
...
| `0x0060–0x00DB` | 124 B | TOTP metadata: 2 B × 62 slots (algorithm + secret_len) | `CONFIG_TOTP_META_START` |
| `0x00E0–0x1FFF` | 7 936 B | 62 credential slots (128 B each = 4 pages × 32 B) | `EEPROM_CREDENTIAL_BASE` |

---

## Credential slot structure

Each of the **62 credential slots** occupies **4 consecutive 32-byte EEPROM pages** (128 bytes total):
...
- The encryption uses the device-wide AES master key and IV
